---
name: nukeviet-theme
description: Thiết kế giao diện NukeViet 4.x — cấu trúc theme, layout, block position, XTemplate. Load khi tạo theme, thêm block, tùy biến giao diện module.
allowed-tools: Read, Write, Glob, Bash
---

# Skill: Theme NukeViet 4.x
...
## Layout (Bootstrap 24 cột — không phải 12)
| layout name | cột |
|---|---|
| `main` | 24 |
| `main-right` | 18-6 |
...
- `modules/` → xóa hết (copy lại từng module khi cần)
...
1. Thêm vào `config.ini`:
```xml
<position><n>Tên hiển thị</n><tag>[TEN_KHOI]</tag></position>
```
Tag: **in hoa**, chữ/số/gạch dưới — vd: `[BOTTOM_CONTENT]`, `[BANNER_TOP]`

2. Đặt tag vào file tpl tương ứng:
```html
<div class="container">[BOTTOM_CONTENT]</div>
```
3. Xóa cache → bật kéo thả block để kiểm tra
